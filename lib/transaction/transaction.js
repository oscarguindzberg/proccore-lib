...
Transaction.prototype.invalidSatoshis = function() {
  ...
  for (var i = 0; i < this.outputs.length; i++) {
    //log.info('this.outputs[i]._satoshis is BigNumber: ' + (this.outputs[i]._satoshis instanceof BigNumber));
    log.info('this.outputs[i]._satoshis is BN: ' + (this.outputs[i]._satoshis instanceof BN));
...
Transaction.prototype.set_log = function(l){
    log = l;
    log.info('log was set!');
}
...
Transaction.prototype.getSerializationError = function(opts) {
  ...
  log.info('getSerializationError: 1');

  if (this.invalidSatoshis()) {
    return new errors.Transaction.InvalidSatoshis();
  }

  log.info('getSerializationError: 2');
  ...
  log.info('getSerializationError: 3: ' + JSON.stringify(unspentError));
...
Transaction.prototype._hasFeeError = function(opts, unspent) {
  log.info('_hasFeeError: ' + JSON.stringify(this._fee) + ', ' + JSON.stringify(unspent));
  if (!_.isUndefined(this._fee) && !this._fee.isEqualTo(unspent)) {
  ...
  if (!opts.disableSmallFees) {
    var est = this._estimateFee();
    var minimumFee = Math.ceil(est / Transaction.FEE_SECURITY_MARGIN);
    log.info('est: ' + est);
    log.info('minimumFee: ' + minimumFee);
...
Transaction.prototype.from = function(utxo, pubkeys, threshold) {
  if (_.isArray(utxo)) {
    var self = this;
    _.each(utxo, function(sutxo) {
      sutxo.get_log = utxo.get_log;
      self.from(sutxo, pubkeys, threshold);
    });
    return this;
  }
...
Transaction.prototype._getOutputAmount = function() {
  if (_.isUndefined(this._outputAmount)) {
    var self = this;
    this._outputAmount = new BN();
    _.each(this.outputs, function(output) {
      self._outputAmount = self._outputAmount.add(output.satoshis);
    });
  }
...
Transaction.prototype._getInputAmount = function() {
  if (_.isUndefined(this._inputAmount)) {
    var self = this;
    this._inputAmount = new BN();
    _.each(this.inputs, function(input) {
      if (_.isUndefined(input.output)) {
        throw new errors.Transaction.Input.MissingPreviousOutput();
      }
      self._inputAmount = self._inputAmount.add(input.output.satoshis);
    });
  }
...
Transaction.prototype._updateChangeOutput = function() {
  ...
  var fee = this.getFee();
  var changeAmount = available.sub(new BN(fee.toFixed()));
  if (changeAmount.cmp(new BN('0')) > 0) {
